...
public:
    ...
    int solve(string s, map<string, int> m)
    {
        ...
        {
            vector<string> expr=parse(s.substr(pos+1));
            string op=s.substr(0, pos);
            if(op=="let")
            {
                for(int i=0;i*2+2<expr.size();i++)
                {
                    m[expr[i*2]]=solve(expr[i*2+1], m);
                }
                
                res=solve(expr.back(), m);
            }
            else if(op=="add")
            {
                res = solve(expr[0], m)+solve(expr[1], m);
            }
            else if(op =="mult")
            {
                res = solve(expr[0], m) * solve(expr[1], m);
