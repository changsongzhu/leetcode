...
Note:
Path Pa1, Pa2, ..., Pan is lexicographically smaller than Pb1, Pb2, ..., Pbm, if and only if at the first iwhere Pai and Pbi differ, Pai < Pbi; when no such i exists, then n < m.
...
public:
  vector <int> cheapestJump(vector<int> A, int B) {
      ...
      int i=0;
      for(i=0;i<A.size()-1&&A[i]>0;i=next[i])
          res.push_back(i+1);
      if(i==A.size()-1&&A[i]>0)
